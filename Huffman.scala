...
object Huffman {
  ...
  /**
    ...
    *   }
    */
  type Pairs = List[(Char, Int)]
  // count how many times (n) each char (c) is used in chars and return a list of pairs (c, n)
  def times(chars: List[Char]): Pairs = {
    def update(c: Char, list: Pairs): Pairs = list match {
      case List() => List((c,1))
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: Pairs): List[Leaf] = 
  ...
  /**
    ...
    */
  type CodeTrees=List[CodeTree]
  def until(condition: (CodeTrees)=>Boolean, operation: (CodeTrees)=>CodeTrees)(trees: CodeTrees): CodeTrees = 
    if (condition(trees)) trees
    else until(condition, operation)(operation(trees))
  
  /**
    ************************************************
    **** The culmination of our efforts so far! ****
    ************************************************
    * 
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def recurse(subtree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = subtree match {
      ...
      case Fork(l,r,cs,w) => bits match {
        ...
        case x::bs => throw new Error("bit, but not 0 or 1:" + x + "!")
      }
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def recurse(tree: CodeTree, acc: CodeTable, bits: List[Bit]): CodeTable = tree match {
      ...
      case Fork(l,r,_,w) => recurse(l,acc,0::bits) ::: recurse(r,acc,1::bits)
    }
